      ...
      words.each do |key|
        ...
        title_image.combine_options do |i|
          ...
          if custom_font_size
            ...
            Helper.log.debug "Using custom font size #{custom_font_size}" if $verbose
          else
      ...
      if fonts
        fonts.each do |font|
          if font['supported']
            ...
            # No `supported` array, this will always be true
            Helper.log.debug "Found a fontSize with no list of supported languages, using this now" if $verbose
            return scaledFontSize(font["fontSize"])
          end
        end
      end

      Helper.log.debug "No custom fontSize specified for #{screenshot}, using the default one" if $verbose
      return nil
    end    
  end
end
